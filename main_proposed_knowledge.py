...
def train(model_seg, model_ae, train_loader, epoch,
    ...
    for i, (input, target, _) in enumerate(train_loader):
        ...
        if args.seg_loss_function == 'bce_logit':
        ...
        loss_seg = (float(args.knowledge_alpha)*(loss_seg))
...
def validate(model_seg, model_ae, val_loader, epoch,
             criterion_seg, criterion_ae,
             logger, logger_ae):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, ori_img) in enumerate(val_loader):
            ...
            output_seg, _ = model_seg(input)
            output_ae, _ = model_ae(target)

            loss_seg = criterion_seg(output_seg, output_ae)

            # seg-log
            iou, dice = performance(output_seg, target, dist_con=False)
            losses.update(loss_seg.item(), input.size(0))
            ious.update(iou, input.size(0))
            dices.update(dice, input.size(0))
