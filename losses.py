...
def JSDLoss(f_real, f_synt):
    f_real = f_real.contiguous().view(f_real.shape[0], -1)
    f_synt = f_synt.contiguous().view(f_synt.shape[0], -1)
    f_num_features = f_real.size()[1]
    #f_num_features f= f_real.cpu().data.numpy().shape[1]
    batch_size = f_real.size()[0]
    ipdb.set_trace()

    identity = torch.autograd.Variable(torch.eye(f_num_features)*0.1).cuda()
    ...
    cov_mat_f_real = (f_real_xx / (batch_size-1)) + identity
    cov_mat_f_synt = (f_synt_xx / (batch_size-1)) + identity
...
def JSDLoss(batch_size, f_real, f_synt):
    ...
    f_num_features = f_real.cpu().data.numpy().shape[1]
    ipdb.set_trace()



    identity = torch.autograd.Variable(torch.eye(f_num_features).cuda()*0.1).cuda()
...
class Focal_tversky_loss(nn.Module):
    ...
    def __init__(self,tversky_loss,gamma):
    ...
    def forward(self, input, target):
        ...
        x = self.tversky_loss(input, target)**self.gamma
...
class ClDice(nn.Module):
    ...
    def __init__(self,bce_loss,dice_loss,alpha=1,beta=1,):
    ...
    def forward(self, input, target):
        ...
        x1 = self.alpha*self.bce_loss(input,target)
        x2 = self.beta*self.dice_loss(input, target)
        cldice_loss = (x1) + (x2)
