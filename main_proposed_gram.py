...
parser.add_argument('--style-alpha', default=1e-7, type=float)
parser.add_argument('--style-direct',default='unidirectional',
                    choices=['unidirectional','Bidirectional'],type=str)
...
def main():
    ...
    # 2.model_select
    model_seg  = Unet2D_style_spinal(
        in_shape=(1, args.input_size, args.input_size),style=args.style)
    model_seg=model_seg.cuda()

    model_ae = ae_spinal_style(
        in_shape=(1, args.input_size, args.input_size),style=args.style)
...
def train(model_seg, model_ae, train_loader, epoch,
    ...
    for i, (input, target, _) in enumerate(train_loader):
        ...
        loss_style = 0
        for j in range(len(bottom_seg)):
            loss_mse = nn.MSELoss().cuda()
            if args.style_direct == 'unidirectional':
                seg_gram = gramMatrix(bottom_seg[j])
                ae_gram = gramMatrix(bottom_ae[j].detach())
                loss_style_per_layer = loss_mse(seg_gram, ae_gram)
            else:
                "Bidirectional"
                loss_style_per_layer = loss_mse(bottom_seg[j], bottom_ae[j])

            loss_style_per_layer = float(args.style_alpha) \
                                   * loss_style_per_layer

            loss_style += loss_style_per_layer

        loss_style = loss_style / len(bottom_seg)
        total_loss = (loss_seg) + (loss_style)


        print('loss_seg : ', loss_seg)
        print('loss_ae : ', loss_ae)
        print('alpha * style_loss_embedding : ', loss_style)
        ...
        embedding_losses.update(loss_style.item(), input.size(0))
        ...
        if args.style_direct == 'Bidirectional':
...
def embedding_loss(embedding_loss, criterion_embedding,
                   bottom_seg, bottom_ae, detach=False):
    if detach == True:
